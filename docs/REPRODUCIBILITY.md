...
- **Demo video hardware (the only device in the demo):** **Samsung Galaxy S25 Ultra** — Snapdragon 8 Elite for Galaxy CPU, Adreno 830 GPU (QVAC uses the `android-arm64` Vulkan/OpenCL backends), 12 GB RAM, 256 GB+ storage. The recorded demo is the **solo tier**: OCR, DDInter grounding, and the MedPsy explanation all run on the phone, offline.
- **Optional mesh hardware (NOT in the demo video):** consumer = **Google Pixel 10 Pro XL** (GPU-accelerated); anchor = an **RTX 5060 / 32 GB** box. Note: the stock `linux-x64` QVAC build ships **no** CUDA/Vulkan backend, so the anchor runs the 4B on **CPU** (the GPU is not used there). See [`MESH-RUNBOOK.md`](MESH-RUNBOOK.md).
